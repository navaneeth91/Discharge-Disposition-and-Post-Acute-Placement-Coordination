...
namespace DischargeDisposition_Backend.Hospital.Services
{
        ...
        : IDispositionDecisionService
    {
        ...
        public async Task<ApiResponse<DispositionDecisionResponse>>
            CreateAsync(CreateDispositionDecisionRequest request)
        {
            ...
            return new ApiResponse<DispositionDecisionResponse>
            {
                Success = true,
                StatusCode = 201,
                Message = "Disposition decision created successfully",
    ...
    GetByPatientIdAsync(int patientId)
        {
            ...
            if (decision == null)
            {
                return new ApiResponse<DispositionDecisionDetailsResponse>
                {
                    Success = false,
                    StatusCode = 404,
                    Message = "Disposition decision not found"
                };
            }
            ...
            return new ApiResponse<DispositionDecisionDetailsResponse>
            {
                Success = true,
                StatusCode = 200,
                Message = "Disposition decision retrieved successfully",
                Data = new DispositionDecisionDetailsResponse
                {
                    DecisionId = decision.DecisionId,
                    PatientId = decision.PatientId,
                    PatientName =
                        $"{decision.patient.FirstName} {decision.patient.LastName}",

                    DispositionTypeId =
                        decision.DispositionTypeId,

                    DispositionTypeName =
                        decision.dispositionType.DispositionName,

                    ClinicianId =
                        decision.ClinicianId,

                    ClinicianName =
                        $"{decision.clinician.FirstName} {decision.clinician.LastName}",
    ...
    UpdateDecisionAsync(
        ...
        {
            var decision =
                await _repository.GetByDecisionIdAsync(decisionId);
            ...
            await _repository.UpdateDecisionAsync(decision);

            return new ApiResponse<DispositionDecisionResponse>
            {
                Success = true,
                StatusCode = 200,
                Message = "Disposition decision updated successfully",
                Data = new DispositionDecisionResponse
                {
                    DecisionId = decision.DecisionId,
                    PatientId = decision.PatientId,
